...
const sdk = await iSelfieCardioSDK({
apiKey: "your-api-key",
appUserId: "user-id",
environment: "dev", // prod
options: {
    displayResults: false, // Display results after test
    enablePDFSharing: false, // Allow sharing results as PDF
    timezone: 'Etc/UTC', // Time zone for test
    disableAudio: false, // Disable audio during the test
    language: 'en', // Language for the test interface
    isDarkMode: false, // Use dark mode by default
    disableMotionDetection: false, // Disable motion detection during the test (if false, motion detection is enabled)
    disableLightCheck: false, // Disable light level check during the test (if false, light level will be checked)
    disableDistanceCheck: false, // Disable distance check during the test (if false, user’s distance from the camera will be checked)
    terminateTestIfConditionPersisted: false, // Terminate the test if certain failure conditions persist during the test
    testTerminationTimeOut: 7, // Timeout in seconds before terminating the test if a persistent condition is detected
    instructionPage: {
    hidden: false, // Display an instruction page before the test starts
    instructionContent: null, // Customize instruction page content
    },
},
styles: {
    pageBackgroundColor: '', // Page background color
    cardBackgroundColor: '', // Card background color
    cardHeaderBackgroundColor: '', // Card header color
...
(async function() {
    ...
    environment: "dev", // prod
    options: {
        displayResults: false, // Display results after the test is completed
        enablePDFSharing:false, // Allow users to share their test results as a PDF document
        timezone: 'Etc/UTC', // Time zone used for displaying timestamps in the test results
        disableAudio: false, // Disable audio during the test
        language: 'en', // Language used for the test interface and instructions
        isDarkMode: true, // Enable dark mode for the test UI by default
        disableMotionDetection: false, // Disable motion detection during the test (if false, motion detection is enabled)
        disableLightCheck: false, // Disable light level check during the test (if false, light level will be checked)
        disableDistanceCheck: false, // Disable distance check during the test (if false, user’s distance from the camera will be checked)
        terminateTestIfConditionPersisted: false, // Terminate the test if certain failure conditions persist during the test
...
**`timezone`**: (string)
Sets the time zone for scheduling and timestamping the test. Use IANA time zone format: [IANA Time Zones Documentation](https://timeapi.io/documentation/iana-timezones).  
**Default**: `"Etc/UTC"`
